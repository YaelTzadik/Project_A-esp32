...
vector<double> FaceRecognizer::distances_from_point(const vector<pair<int16_t, int16_t>>* matrix, int index = 0) {
  vector<double> distances;

  pair<int16_t, int16_t> base_point = (*matrix)[index];
...
vector<double> FaceRecognizer::get_distance_faces(const vector<pair<int16_t, int16_t>>* points, const vector<pair<int16_t, int16_t>>* target_points) {
  vector<pair<int16_t, int16_t>> points_stretched = stretch_and_align_points(points);
  Serial.println(target_points->size());
  vector<pair<int16_t, int16_t>> target_points_stretched = stretch_and_align_points(target_points);
...
String FaceRecognizer::identify(const vector<String>* ids, const vector<vector<pair<int16_t, int16_t>>>* target_points, const vector<pair<int16_t, int16_t>>* face) {
  double min_tot_dist = 1400;
  int min_tot_id = -1;
  // Iterate through each vector of pairs using a for loop
  for (size_t i = 0; i < target_points->size(); ++i) {
    const std::vector<std::pair<int16_t, int16_t>>& target = (*target_points)[i];
...
FaceRecognizer::stretch_and_align_points(const vector<pair<int16_t, int16_t>>* points) {
  ...
  vector<pair<int16_t, int16_t>> stretched_points;
  for (const auto& point : *points) {
