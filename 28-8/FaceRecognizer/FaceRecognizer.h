...
private:
    ...
    vector<vector<int>> stretch_and_align_points(const vector<pair<int, int>> &points);


    vector<double> distances_from_point(const vector<vector<int>> &matrix, int index);

    std::vector<double> get_distance_faces(const vector<pair<int, int>> &points, const vector<pair<int, int>> &target_points);

public:
    bool identify(const vector<string> &ids, const vector<vector<pair<int, int>>> &target_points, const vector<pair<int, int>> &face);
